...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $consulta = $conexion->prepare("SELECT id, username, email, password, rol FROM usuarios WHERE username = ? OR email = ?");
    $consulta->bind_param("ss", $usernameOrEmail, $usernameOrEmail);
    $consulta->execute();
    $consulta->store_result();

    if ($consulta->num_rows > 0) {
        $consulta->bind_result($id, $username, $email, $hashedPassword, $rol);
        $consulta->fetch();
